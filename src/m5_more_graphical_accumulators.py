...
def draw_squares_from_circle(n, circle, window):
    ...
    for k in range(n):
        center_point_x = circle.center.x-circle.radius+(1/2)**k*circle.radius
        center_point_y = circle.center.y-circle.radius+(1/2)**k*circle.radius
        center_point = rg.Point(center_point_x, center_point_y)
        edge = (1/2) ** k * circle.radius*2
        square = rg.Square(center_point, edge)
...
def run_test_draw_circles_from_rectangle():
    ...
    title = title + 'two seperate rectangles with circles on the left and and the top of the rectangles'
    window1 = rg.RoseWindow(600, 600, title)

    # Test 1:
    rectangle = rg.Rectangle(rg.Point(400, 100), rg.Point(430, 120))
    rectangle.fill_color = 'blue'
    rectangle.outline_color = 'purple'
    draw_circles_from_rectangle(4, 3, rectangle, window1)
    # Test 2:
    rectangle = rg.Rectangle(rg.Point(200, 400), rg.Point(240, 430))
    rectangle.fill_color = 'green'
    rectangle.outline_color = 'pink'
    draw_circles_from_rectangle(5, 2, rectangle, window1)
    window1.close_on_mouse_click()

    title = 'Test 3 of DRAW_CIRCLES_FROM_RECTANGLE: '
    title = title + 'one rectangle with 2 circles on the left and 4 circles at the top'
    window2 = rg.RoseWindow(400, 400, title)

    # Test 3:
    rectangle = rg.Rectangle(rg.Point(100, 100), rg.Point(110, 120))
    rectangle.fill_color = 'yellow'
    rectangle.outline_color = 'red'
    draw_circles_from_rectangle(2, 4, rectangle, window2)
    window2.close_on_mouse_click()
...
def draw_circles_from_rectangle(m, n, rectangle, window):
    ...
    for k in range(m):
        ...
        center_point_x = rectangle.corner_1.x+radius*(2*k+1)
        center_point_y = rectangle.corner_1.y-radius
        center_point = rg.Point(center_point_x, center_point_y)
...
def draw_lines_from_rectangles(rectangle1, rectangle2, n, window):
    ...
    for k in range(n):
        start_point_x = rectangle1.get_center().x-(1/2)*rectangle1.get_width()*k
        start_point_y = rectangle1.get_center().y+(1/2)*rectangle1.get_height()*k
        end_point_x = rectangle2.get_center().x-(1/2)*rectangle1.get_width()*k
        end_point_y = rectangle2.get_center().y+(1/2)*rectangle1.get_height()*k
